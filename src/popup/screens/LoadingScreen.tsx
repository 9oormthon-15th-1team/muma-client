...
function VinylLoading({ count }: { count: number }) {
  return (
    <div className="relative inline-grid place-items-start">
      ...
      <span className="col-start-1 row-start-1 ml-[55px] mt-[54px] text-[17px] font-bold leading-[24px] text-[#424848]">{count}</span>
  ...
  return (
    <ScreenLayout onBack={onBack}>
      ...
      <div className="flex w-full shrink-0 flex-col gap-[var(--spacing-4)]">
        ...
        <p className="m-0 text-[11px] leading-4 text-[var(--color-action-disabled)]">{statusMessage}</p>
      </div>

      {/* Steps checklist */}
      <div className="flex w-full flex-col gap-[var(--spacing-12)] rounded-[var(--radius-12)] border border-[var(--color-bg-secondary)] p-[var(--spacing-16)]">
        {steps.map((step, i) => (
          <div key={i} className="flex items-center gap-[var(--spacing-8)]">
            <CheckCircle checked={step.done} />
            <span className="text-[12px] font-semibold leading-[18px] text-[var(--color-text-inverse)]">{step.label}</span>
          </div>
        ))}
      </div>

      <div className="flex h-[var(--size-button-height)] w-full shrink-0 items-center justify-center">
        <span className="text-[15px] font-semibold leading-[22px] text-[var(--color-bg-secondary)]">잠시만 기다려주세요...</span>
      </div>
    </ScreenLayout>
  )
}
